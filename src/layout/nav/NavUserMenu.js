    ...
    <div>
      <Row className="mb-3 ms-0 me-0">
        <Col xs="12" className="ps-1 mb-2">
          <div className="text-extra-small text-primary">COMPTE</div>
        </Col>
      </Row>
        ...
        <Col xs="6" className="pe-1 ps-1">
          <ul className="list-unstyled">
            <li onClick={HandleLogout}>
              ...
              <span style={{ cursor: 'pointer', color: '#1ea8e7' }} className="align-middle">
                Logout
              </span>
            </li>
          </ul>
...
const NavUserMenuDropdownToggle = React.memo(
  React.forwardRef(({ onClick, expanded = false, user = {} }, ref) => (
    ...
    >
      <img className="profile" alt="yo" src="/img/profile/user.png" />
      <div className="name">{user?.data?.email}</div>
    </a>
  ))
);
